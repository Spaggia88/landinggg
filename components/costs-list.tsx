        ...
        <table className="table-auto w-full text-sm">
          ...
          <tbody>
            {costs.map((cost, index) => (
              <tr key={index} className="relative group odd:bg-gradient-to-tr from-slate-100 to-slate-50 dark:from-slate-800/80 dark:to-slate-900">
                <th scope="row" className="text-left font-normal px-4 py-5 first:rounded-l-lg last:rounded-r-lg">
                ...
                <td className="font-semibold text-right px-4 py-5 first:rounded-l-lg last:rounded-r-lg w-[1%] after:content-['Details_->'] after:absolute after:inset-0 after:pr-4 after:flex after:items-center after:justify-end after:bg-gradient-to-l after:from-white group-odd:after:from-slate-50 dark:after:from-slate-950 group-odd:dark:after:from-slate-900 after:to-50% after:rounded-lg after:text-blue-500 after:font-medium after:tracking-normal after:whitespace-nowrap after:opacity-0 group-hover:after:opacity-100 after:transition">
                  <span className="group-hover:opacity-0 transition-opacity">{formatValue(cost.price)}</span>
                </td>                  
              </tr>
            ))}
          </tbody>
